...
void main() {
    ...
    // Distance-based falloff from the live cursor position — used to make
    // the field visibly react to the mouse rather than just drift on its own.
    float aspect = u_resolution.x / max(u_resolution.y, 1.0);
    vec2 mouseDelta = screenUv - u_mouse;
    mouseDelta.x *= aspect;
    float mouseDist = length(mouseDelta);
    float mouseGlow = smoothstep(0.75, 0.0, mouseDist);
    ...
    // Gently drag the pattern toward the cursor for a continuous, interactive feel.
    uv += (u_mouse - 0.5) * 0.12;

    float n1 = noise(uv * 1. + t);
    float n2 = noise(uv * 2. - t);
    float angle = n1 * TWO_PI;
    float distortionBoost = 1.0 + 1.5 * mouseGlow;
    uv.x += 4. * u_distortion * n2 * cos(angle) * distortionBoost;
    uv.y += 4. * u_distortion * n2 * sin(angle) * distortionBoost;
    ...
    // Soft warm bloom that tracks the cursor, layered on top of the base field.
    vec3 litColor = color_mix.rgb + mouseGlow * 0.1;

    fragColor = vec4(litColor, color_mix.a);
}
`;
